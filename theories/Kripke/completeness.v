...
Class segment Γ : Type :=
  { head : @Ensemble form ;
    ...
    segClosed : forall th, (th = head \/ tail th) -> restr_closed (Clos Γ) th ;
...
Hypothesis segment_prf_irrel : forall Γ (s0 s1 : segment Γ),
                (@head _ s0) = (@head _ s1) ->
                (@tail _ s0) = (@tail _ s1) ->
                s0 = s1.
...
Lemma cexpl_segClosed Γ : forall th, (th = (Clos Γ) \/ (Singleton _ (Clos Γ)) th) ->  restr_closed (Clos Γ) th.
Proof.
...
- assert (Jψ1: Clos Γ ψ1).
  ...
  eapply MP. eapply MP. eapply MP. apply Ax ; left ; eapply IA8 ; reflexivity.
...
- assert (Jψ1: Clos Γ ψ1).
  ...
  apply Ax ; left ; eapply IA6 ; reflexivity. apply Id. exact H0.
  apply IHψ2 ; auto. apply segClosed ; auto. eapply MP.
  apply Ax ; left ; eapply IA7 ; reflexivity. apply Id. exact H0.
(* Or ψ1 ψ2 *)
- assert (Jψ1: Clos Γ ψ1).
  apply Incl_ClosSubform_Clos. unfold In. exists (ψ1 ∨ ψ2). split ; simpl ; auto. right.
  apply in_or_app ; left ; destruct ψ1 ; simpl ; auto.
  ...
  apply Ax ; left ; eapply IA3 ; reflexivity. apply Id. exact H0.
  apply IHψ2 in H0 ; auto. apply segClosed ; auto. eapply MP.
  apply Ax ; left ; eapply IA4 ; reflexivity. apply Id. exact H0.
- assert (Jψ1: Clos Γ ψ1).
  apply Incl_ClosSubform_Clos. unfold In. exists (ψ1 ∨ ψ2). split ; simpl ; auto. right.
  apply in_or_app ; left ; destruct ψ1 ; simpl ; auto.
  assert (Jψ2: Clos Γ ψ2).
...
- assert (Jψ1: Clos Γ ψ1).
  ...
  assert (~ CKH_prv (Union _ head (Singleton _ ψ1)) ψ2).
  intro. apply CKH_Deduction_Theorem in H2. apply H1. apply segClosed ; auto.
...
- assert (Jψ1: Clos Γ ψ1).
  ...
  apply IHψ2 ; auto.
  assert (CKH_prv head ψ2). eapply MP. apply Id ; exact H0.
...
- assert (Jψ: Clos Γ ψ).
  ...
  assert (~ CKH_prv (fun x => exists A, (@head _ s) (☐ A) /\ x = A) ψ).
  intro. apply H1. apply segClosed ; auto. apply K_rule in H2.
  apply (CKH_monot _ _ H2). intros B HB. unfold In in *.
  ...
  assert (forall th : Ensemble form, th = head \/ (fun x => x = Γ0 \/ x = (Clos Γ)) th -> restr_closed (Clos Γ) th).
  ...
  assert (K1: forall th : Ensemble form, th = Γ0 \/ Singleton (Ensemble form) (Clos Γ) th -> restr_closed (Clos Γ) th).
...
- assert (Jψ: Clos Γ ψ).
  ...
  simpl in H0.
  destruct (classic (head (⬦ ψ))) ; auto. exfalso.
  assert (R0: forall B, head (⬦ B) -> ~ CKH_prv (fun x => exists A, B = x \/ ((@head _ s) (☐ A) /\ x = A)) ψ).
  intros B HB D. apply H1. apply segClosed ; auto.
  apply CKH_monot with (Γ1:= Union _ (fun x : form => exists A : form, head (☐ A) /\ x = A) (Singleton _ B)) in D.
  apply CKH_Deduction_Theorem in D. eapply MP. eapply MP. apply Ax ; right ; eapply MAK2 ; reflexivity.
  apply K_rule in D. apply (CKH_monot _ _ D). intros C HC. unfold In in *.
  ...
  apply Incl_ClosSubform_Clos. exists (☐ D).
  split ; cbn ; auto. apply segInclClos in HD0 ; auto. right ; destruct D ; cbn ; auto.
  remember (fun x => exists B (P: head (⬦ B)), x = proj1_sig (Lindenbaum _ _ ψ Jψ (R2 _ P) (R0 _ P))) as LindSet.
  ...
  destruct (Lindenbaum Γ (fun x : form => exists A : form, B = x \/ head (☐ A) /\ x = A) ψ Jψ 
  (R2 B HB) (R0 B HB)) as (Γ0 & L0 & L1 & L2 & L3 & L4) ; cbn in *. apply L1 in HC ; auto.
  assert (forall th : Ensemble form, th = head \/ LindSet th -> restr_closed (Clos Γ) th).
  ...
  intros.
  destruct (Lindenbaum Γ (fun x : form => exists B : form, A = x \/ head (☐ B) /\ x = B) ψ Jψ
...
- assert (Jψ: Clos Γ ψ).
  ...
  assert (forall th : Ensemble form, th = x \/ Singleton (Ensemble form) (Clos Γ) th -> restr_closed (Clos Γ) th).
...
Theorem QuasiCompleteness : forall Γ φ,
    ~ CKH_prv Γ φ -> ~ loc_conseq Γ φ.
Proof.
intros Γ φ WD H.
apply Lindenbaum_segment with (Γ:=Union _ Γ (Singleton _ φ)) in WD ; auto.
...
Theorem Strong_Completeness : forall Γ φ,
    loc_conseq Γ φ -> CKH_prv Γ φ.
Proof.
intros Γ φ LC. pose (QuasiCompleteness Γ φ).
destruct (classic (CKH_prv Γ φ)) ; auto. exfalso.
apply n ; assumption.
Qed.
