...
Definition opt_weight (o : option form) :=
  match o with
  | None => 0
  | Some φ => weight φ
  end.
...
Definition pointed_env := (list (form) * option form)%type.

Definition listform_opt_add2 (Γ : list form) o : list form :=
  match o with
  | None => Γ
  | Some φ => Γ • φ • φ
  end.
...
Definition pointed_env_order (pe1 : pointed_env) (pe2 : pointed_env) :=
  env_order (listform_opt_add2 (fst pe1) (snd pe1)) (listform_opt_add2 (fst pe2) (snd pe2)).

Lemma wf_pointed_order : well_founded pointed_env_order.
Proof. apply well_founded_ltof. Qed.

Definition pointed_env_ms_order (Γφ Δψ : env * option form) :=
...
Lemma l_open_boxes_env_order Δ : (l_open_boxes Δ) ≼ Δ.
Proof.
unfold env_order_refl.
induction Δ as [|φ Δ]; trivial.
simpl. dependent destruction φ; try (rewrite env_weight_add ; lia).
do 2 rewrite env_weight_add ; cbn ; lia.
Qed.

Global Hint Resolve l_open_boxes_env_order : order.
...
Lemma env_order_1' Δ Δ' φ1 φ: weight φ1 < weight φ ->
  (Δ' ≼ Δ) -> Δ' • φ1 ≺ Δ • φ.
Proof.
intros Hw1 Hor.
unfold env_order, ltof. repeat rewrite env_weight_add.
...
Lemma pointed_env_order_bot_R pe Δ φ: (pe ≺· (Δ, Some ⊥)) -> pe ≺· (Δ, Some φ).
Proof.
...
Lemma pointed_env_order_None_R pe Δ φ: (pe ≺· (Δ, None)) -> pe ≺· (Δ, φ).
Proof.
intro Hlt. destruct pe as (Γ, ψ). unfold pointed_env_order. simpl.
eapply env_order_lt_le_trans. exact Hlt. simpl.
destruct φ as [ φ | ] ; [ | left ; auto ].
cbn. do 2 (apply env_order_env_order_refl, env_order_0').
left ; auto.
Qed.

Hint Resolve pointed_env_order_bot_R : order.

Lemma pointed_env_order_bot_L pe Δ φ: ((Δ, Some φ) ≺· pe) -> (Δ, Some ⊥) ≺· pe.
Proof.
intro Hlt. destruct pe as (Γ, ψ). unfold pointed_env_order. simpl.
...
Lemma pointed_env_order_None_L pe Δ φ: ((Δ, φ) ≺· pe) -> (Δ, None) ≺· pe.
Proof.
intro Hlt. destruct pe as (Γ, ψ). unfold pointed_env_order. simpl.
eapply env_order_le_lt_trans; [|exact Hlt]. simpl.
destruct φ as [ φ | ] ; [ | left ; auto ].
...
Lemma list_filter_order (P : form -> bool) l : List.filter P l ≼ l.
Proof.
induction l ; cbn ; [ left ; auto | ].
destruct (P a).
+ order_tac.
+ order_tac.
Qed.
...
Lemma elements_open_boxes_order Γ : elements (⊗ Γ) ≼ elements (Γ).
Proof.
rewrite elements_open_boxes.
apply env_order_refl_trans with (List.filter is_box (elements Γ)) ; [ apply open_boxes_env_order | ].
apply list_filter_order.
Qed.