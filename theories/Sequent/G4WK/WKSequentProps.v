...
Theorem emptyweakR Γ : Γ ⊢ None -> forall φ, Γ ⊢ φ.
Proof with (auto with proof).
intro H. remember None as o. revert Heqo.
induction H; intro φ'; auto with proof; try (exch 0; auto with proof) ; try discriminate ; subst.
intros ; apply DiamL ; auto.
Qed.
...
- cbn in Hp. exch 0. apply weakening. apply DiamL.
  apply emptyweakR with (φ:=oud (Some ψ)) in Hp ; auto.
Qed.

Global Hint Resolve ImpR_rev : proof.

Theorem generalised_axiom  Γ φ : Γ • φ ⊢ Some φ.
Proof with (auto with proof).
...
Lemma exfalso  Γ φ: Γ ⊢ Some ⊥ -> Γ ⊢ Some φ.
Proof.
intro Hp. dependent induction Hp; try tauto; auto with proof.
cbn in Hp. apply DiamL, emptyweakR ; auto.
Qed.

Global Hint Immediate exfalso : proof.

Lemma AndR_rev  {Γ φ1 φ2} : Γ ⊢ Some (φ1 ∧ φ2) -> (Γ ⊢ Some φ1) * (Γ ⊢ Some φ2).
Proof.
...
Lemma weak_ImpL  Γ φ ψ θ : Γ ⊢ Some φ -> Γ•ψ ⊢ θ -> Γ • (φ → ψ) ⊢ θ.
Proof with (auto with proof).
intro Hp. remember (Some φ) as o. revert φ Heqo ψ θ.
induction Hp; intros φ0 Heq ψ0 θ0 Hp' ; inversion Heq ; subst ; auto with proof.
- exch 0; constructor 4; exch 1; exch 0.
  apply IHHp ; auto. exch 0; exch 1. apply AndL_rev. exch 0 ; auto.
- apply ImpLOr. exch 0...
- exch 0; constructor 7; exch 0.
  + apply IHHp1 ; auto. exch 0. eapply fst, OrL_rev. exch 0. exact Hp'.
  + apply IHHp2 ; auto. exch 0. eapply snd, OrL_rev. exch 0. exact Hp'.
- exch 0; exch 1. constructor 9. exch 1; exch 0.
  apply IHHp ; auto. exch 0 ; exch 1 ; apply ImpLVar_rev ; auto ; exch 1 ; exch 0 ; auto.
- exch 0. apply ImpLAnd. exch 0.
  apply IHHp ; auto. exch 0 ; apply ImpLAnd_rev ; exch 0 ; auto.
- exch 0. apply ImpLOr. exch 1; exch 0. apply IHHp ; auto.
  exch 0 ; exch 1 ; apply ImpLOr_rev ; exch 0 ; auto.
- exch 0. apply ImpLImp; exch 0. auto with proof. apply IHHp2 ; auto. exch 0.
  eapply ImpLImp_prev. exch 0. eassumption.
- exch 0. apply ImpBox.
  + rewrite open_boxes_add_f ; auto.
  + exch 0. apply IHHp2 ; auto. exch 0. apply ImpLBox_prev with φ1. exch 0. exact Hp'.
- exch 0. exch 1. apply ImpDiam.
  + rewrite open_boxes_add_f ; auto.
  + exch 1. exch 0. apply IHHp2 ; auto. exch 0. exch 1. apply ImpLDiam_prev with φ1. exch 1. exch 0. exact Hp'.
- destruct φ0.
  1-6: cbn in Hp ; exch 0 ; apply DiamL ; rewrite open_boxes_add_f ; auto ; apply emptyweakR ; auto.
  cbn in Hp. apply ImpDiam ; auto.
Qed.
...
- case (decide (ψ = (#p → φ))); intro Heq.
  + subst. exhibit Hin' 0. rewrite union_difference_R in Hin' by ms.
    assert(Hcut : (((Γ•Var p) ∖ {[Var p → φ]}•(Var p → φ)) ⊢ oψ)); [|peapply Hcut].
    ...
    * subst.
      ...
      exhibit Hin''' 1. apply ImpDiam.
        ...
        -- case (decide (φ2 = ◊ χ)); intro Heq1. (* Feels unnecessary... *)
          ...
          ++ assert ((Γ • ◊ χ • φ2) ∖ {[◊ χ]} ⊢WK oψ). apply IHh with Hp2 ; auto ; try lia.
...
Proof.
intros H1 H2. apply contraction, weak_ImpL; auto with proof.
exch 0 ; apply weakening ; auto.
Qed.
...
- case (decide ((φ1 ∧ φ2 → φ3) = (φ → ψ))); intro Heq0.
  + dependent destruction Heq0; subst.
    ...
    peapply (IHw (Γ•(φ2 → ψ)) φ2 ψ oψ); [lia|ms|].
    peapply (IHw (Γ•(φ1 → φ2 → ψ)) φ1 (φ2 → ψ) oψ); [lia|ms|trivial].
  + forward. apply ImpLAnd. backward. apply IHHp; trivial. ms.
- case (decide ((φ1 ∨ φ2 → φ3) = (φ → ψ))); intro Heq0.
...
Lemma OrR_idemp  Γ ψ : Γ ⊢ Some (ψ ∨ ψ) -> Γ ⊢ Some ψ.
...
Lemma var_not_tautology  v: (∅ ⊢ Some (Var v)) -> False.
Proof.
intro Hp.
remember ∅ as Γ.
dependent induction Hp;
...
Lemma box_var_not_tautology v: (∅ ⊢ Some (□ (Var v))) -> False.
Proof.
intro Hp.
remember ∅ as Γ.
dependent induction Hp;
...
Lemma diam_var_not_tautology v: (∅ ⊢ Some (◊ (Var v))) -> False.
Proof.
intro Hp.
remember ∅ as Γ.
dependent induction Hp;
...
Lemma box_bot_not_tautology: (∅ ⊢ Some (□⊥)) -> False.
Proof.
 intro Hp.
...
Lemma diam_bot_not_tautology : (∅ ⊢ Some (◊ ⊥)) -> False.
Proof.
intro Hp.
remember ∅ as Γ.
dependent induction Hp;
...
Lemma weight_tautology  (φ : form) : (∅ ⊢ Some φ) -> {φ = ⊤} + {3 ≤ weight φ}.
Proof.
intro Hp.
destruct φ.
