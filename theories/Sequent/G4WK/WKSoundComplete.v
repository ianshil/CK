...
Require Import WKSequents WKSequentProps WKOrder WKCut. (* Sequent calculus *)
Require Import CKH_export. (* Hilbert calculus *)
Require Import WKDecisionProcedure.

Section Soundness.

(* First we need some theorems about WKH_prv relating to the 
   syntactic definitions pertaining to G4WK. *)

Lemma open_boxes_K_rule (Γ : env) φ :
  WKH_prv (fun γ => γ ∈ (⊗ Γ)) φ ->  WKH_prv (fun γ => γ ∈ Γ) (□ φ).
Proof.
...
Lemma open_boxes_Diam_rule (Γ : env) φ ψ : (◊ ψ) ∈ Γ ->
  WKH_prv (fun γ => γ ∈ ((⊗ Γ) • ψ)) φ ->  WKH_prv (fun γ => γ ∈ Γ) (◊ φ).
Proof.
...
Definition opt_to_form o : form :=
  match o with
  | None => ⊥
  | Some φ => φ
  end.

Theorem G4WK_sound_CKH Γ φ : Γ ⊢ φ -> WKH_prv (fun γ => γ ∈ Γ) (opt_to_form φ).
Proof.
induction 1 ; cbn.
...
- destruct oφ ; cbn.
  + destruct f ; cbn in *.
    1-6: eapply MP ; [ eapply Ax ; left ; left ; eapply IA9 ; reflexivity |
         eapply MP ; [ eapply Ax ; right ; reflexivity| 
    ...
    [ apply gmultiset_elem_of_disj_union ; right ; apply gmultiset_elem_of_singleton ; auto | 
      rewrite open_boxes_add_f ; auto ].
  + cbn in *. eapply MP ; [ eapply Ax ; right ; reflexivity| ].
    apply open_boxes_Diam_rule with ψ.
    * apply gmultiset_elem_of_disj_union ; right ; apply gmultiset_elem_of_singleton ; auto.
    * rewrite open_boxes_add_f ; auto.
Qed.
...
Theorem G4WK_compl_WKH Γ φ : WKH_prv (fun γ => γ ∈ Γ) φ -> Γ ⊢ Some φ.
Proof.
enough (WKH_prv (fun γ => γ ∈ Γ) φ -> exists (P : Γ ⊢ Some φ), True).
{ intro D. apply H in D. destruct (Provable_dec (elements Γ) (Some φ)).
  + rewrite (proper_Provable _ (list_to_set_disj (elements Γ))) with (y:=Some φ) ; auto.
    rewrite <- elements_list_to_set_disj ; auto.
  + exfalso. destruct D as [D Vrai] ; auto. apply f.
    rewrite (proper_Provable _ Γ) with (y:=Some φ) ; auto. rewrite <- elements_list_to_set_disj ; auto. }
{ intro. remember (λ γ : form, γ ∈ Γ) as Γ'.
  revert Γ HeqΓ'. induction H ; intros ; subst.
  (* Id *)
  - assert (Γ0 ⊢WK Some A). exhibit H 0. apply generalised_axiom.
    exists H0 ; auto.
  (* Axiom *)
  - destruct H as [H | H] ; [ destruct H ; destruct H ; subst | subst ].
    + assert (Γ0 ⊢WK Some (A0 → B → A0)). do 2 (apply ImpR). exch 0. apply generalised_axiom.
      exists H ; auto.
    + assert (Γ0 ⊢WK Some ((A0 → B → C) → (A0 → B) → A0 → C)). 
      ...
      * exch 1 ; exch 0. apply weak_ImpL.
      ...
      exists H ; auto.
    + assert (Γ0 ⊢WK Some (A0 → A0 ∨ B)). apply ImpR,OrR1 ; apply generalised_axiom.
      exists H ; auto.
    + assert (Γ0 ⊢WK Some (B → A0 ∨ B)). apply ImpR,OrR2 ; apply generalised_axiom.
      exists H ; auto.
    + assert (Γ0 ⊢WK Some ((A0 → C) → (B → C) → A0 ∨ B → C)).
      ...
      * exch 0. apply weak_ImpL ; apply generalised_axiom. }
      exists H ; auto.
    + assert (Γ0 ⊢WK Some (A0 ∧ B → A0)). apply ImpR,AndL ; exch 0 ; apply generalised_axiom.
      exists H ; auto.
    + assert (Γ0 ⊢WK Some (A0 ∧ B → B)). apply ImpR,AndL ; apply generalised_axiom.
      exists H ; auto.
    + assert (Γ0 ⊢WK Some ((A0 → B) → (A0 → C) → A0 → B ∧ C)).
      { do 3 (apply ImpR). apply AndR.
      * exch 1 ; exch 0. apply weak_ImpL ; apply generalised_axiom.
      * exch 0. apply weak_ImpL ; apply generalised_axiom. }
      exists H ; auto.
    + assert (Γ0 ⊢WK Some (⊥ → A0)). apply ImpR,ExFalso. exists H ; auto.
    + assert (Γ0 ⊢WK Some (□ (A0 → B) → □ A0 → □ B)). do 2 (apply ImpR). apply BoxR. repeat rewrite open_boxes_add_t ; auto ; cbn.
      exch 0 ; apply weak_ImpL ; apply generalised_axiom.
      exists ; auto.
    + assert (Γ0 ⊢WK Some (□ (A0 → B) → ◊ A0 → ◊ B)). do 2 (apply ImpR). apply DiamL. repeat rewrite open_boxes_add_t ; auto ; cbn.
      exch 0 ; apply weak_ImpL ; apply generalised_axiom.
      exists H ; auto.
    + assert (Γ0 ⊢WK Some ((◊ ⊥) → ⊥)). apply ImpR,DiamL,ExFalso. exists H ; auto.
  (* MP *)
  ...
  - destruct (IHextCKH_prv (⊗ ∅)) ; auto.
    + apply Extensionality_Ensembles ; split ; intros x Hx.
      ...
      * unfold Ensembles.In in Hx. apply elem_of_open_boxes in Hx as [ψ [H0 H1]] ; subst.
        inversion H1.
    + assert (Γ0 ⊢WK Some (□ A)). rewrite <- gmultiset_disj_union_right_id. apply generalised_weakeningL.
      apply BoxR ; auto.
      exists H1 ; auto. }
Qed.
