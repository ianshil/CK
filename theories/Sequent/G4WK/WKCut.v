...
(** * Cut Admissibility *)
Require Import im_syntax syntax_facts WKOrder.
Require Import WKSequents WKSequentProps.
...
remember (Some φ) as o. destruct HPφ; inversion Heqo ; subst ; simpl in Hw.
...
- apply AndL. apply IHW; auto with proof.
  ...
  + exch 0 ; exch 1 ; apply AndL_rev ; exch 0 ; auto.
- apply OrL_rev in HPψ; apply (IHw φ0); [lia| |]; tauto.
...
(* START *)
  ...
  + forward. apply AndL. apply IHW.
     * unfold pointed_env_ms_order. destruct oθ ; otac Heq.
  ...
  + forward. apply ImpR in HPφ.
       assert(Hin' : (φ ∨ ψ) ∈ ((Γ0 • φ ∨ ψ) ∖ {[φ0→ ψ0]}))
            by (apply in_difference; [discriminate|ms]).
       assert(HPφ' : (((Γ0 • φ ∨ ψ) ∖ {[φ0→ ψ0]}) ∖ {[φ ∨ ψ]} • φ ∨ ψ) ⊢ Some (φ0 → ψ0))
            by (rw (symmetry (difference_singleton _ (φ ∨ψ) Hin')) 0; peapply HPφ).
       assert (HP := (OrL_rev  _ φ ψ (Some (φ0 → ψ0)) HPφ')).
       apply OrL.
      * apply IHW.
        -- rewrite env_replace in Heq by trivial. destruct oθ ; otac Heq.
        -- peapply HP.1.
  ...
  + case (decide (((φ1 ∧ φ2) → φ3)= (φ0 → ψ0))).
      ...
      * intro Hneq. forward. apply ImpLAnd, IHW.
        -- rewrite env_replace in Heq by trivial. destruct oψ ; otac Heq.
...
- remember (Γ • □ φ0) as Γ' eqn:HH.
  ...
  + forward. apply BoxR in HPφ.
       assert(Hin' : (φ ∨ ψ) ∈ ((Γ0 • φ ∨ ψ) ∖ {[□ φ0]}))
            by (apply in_difference; [discriminate|ms]).
       assert(HPφ' : (((Γ0 • φ ∨ ψ) ∖ {[□ φ0]}) ∖ {[φ ∨ ψ]} • φ ∨ ψ) ⊢ Some (□ φ0)).
       { rw (symmetry (difference_singleton _ (φ ∨ ψ) Hin')) 0.
         rw (symmetry Heq) 0.
         exact HPφ. }
       assert (HP := (OrL_rev  _ φ ψ (Some (□φ0)) HPφ')).
       apply OrL.
      * apply IHW.
        -- destruct oθ ; otac Heq.
        -- peapply HP.1.
  ...
  + (* (VIII-b) *)
     ...
     * (* π0 *)
      ...
      -- peapply HPψ1. rewrite open_boxes_remove_t ; auto ; cbn.
         ...
         apply in_map_iff. exists (□ φ0) ; cbn ; split ; auto.
  ...
  + (* (VIII-c) *)
      ...
      apply BoxR. apply (IHw φ0) ; auto. lia.
  + (* (VIII-e) *)
      forward. apply DiamL. apply (IHw φ0) ; auto.
      * lia.
      * apply weakening. peapply HPφ. rewrite Heq. rewrite open_boxes_remove_t ; auto ; cbn.
        rewrite open_boxes_remove_t ; auto ; cbn. rewrite open_boxes_add_f ; auto.
      * exch 0. peapply HPψ. rewrite open_boxes_remove_t ; auto ; cbn.
        ...
        apply elem_of_list_In. apply gmultiset_elem_of_elements ; auto.
(* IX *)
- destruct φ ; cbn in Hw.
  1-6: apply DiamL ; apply emptyweakR ; cbn in HPφ ; auto.
  ...
  + forward. apply AndL. apply IHW.
     * repeat rewrite env_replace in Heq by trivial.
       ...
       destruct oθ ; otac Heq' ; rewrite <- elements_elem_of ; order_tac ; rewrite <- Heq ; ms.
  ...
  + forward. apply DiamL in HPφ.
       assert(Hin' : (φ0 ∨ ψ) ∈ ((Γ0 • φ0 ∨ ψ) ∖ {[◊ φ]}))
            by (apply in_difference; [discriminate|ms]).
       assert(HPφ' : (((Γ0 • φ0 ∨ ψ) ∖ {[◊ φ]}) ∖ {[φ0 ∨ ψ]} • φ0 ∨ ψ) ⊢ Some (◊ φ)).
       { rw (symmetry (difference_singleton _ (φ0 ∨ ψ) Hin')) 0.
         rw (symmetry Heq) 0.
         exact HPφ. }
       assert (HP := (OrL_rev  _ φ0 ψ (Some (◊φ)) HPφ')).
       apply OrL.
      * apply IHW.
        -- repeat rewrite env_replace in Heq by trivial.
           assert (Heq': Γ ≡ ((Γ0 ∖ {[◊ φ]} • φ0 ∨ ψ) ∖ {[ ◊ ψ0 ]})) by ms.
  ...
  + do 2 forward. exch 0. apply ImpLVar, IHW.
      * repeat rewrite env_replace in Heq by trivial.
        ...
        destruct oψ ; otac Heq' ; rewrite <- elements_elem_of ; order_tac ; rewrite <- Heq ; ms.
      * apply imp_cut with (φ := Var p). exch 0. do 2 backward.
      ...
      -- apply IHW.
          ++ repeat rewrite env_replace in Heq by trivial. repeat rewrite env_add_remove by trivial.
             ...
             { pose (gmultiset_elements_disj_union (Γ0 ∖ {[◊ ψ0]} • ◊ ψ0 ) ({[+ ◊ φ1 → φ2 +]})). rewrite p.
               ...
               rewrite <- difference_singleton ; ms. }
             destruct oψ ; otac Heq' ; rewrite <- elements_elem_of. 2,4: ms. 
             all: rewrite env_add_remove by trivial.
             all: epose (@env_order_equiv_right_compat (elements Γ0 • φ2) _ _ H2) ; apply e ;
