...
- destruct ψ ; cbn ; cbn in E0 ; try discriminate ; auto ; apply env_order_0' ; apply l_open_boxes_env_order.
Qed.

Equations e_rule_12 {Δ : list form} {ϕ : form}
  {E : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form}
  {A : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form}
...
Qed.
Next Obligation.
rewrite rm_comm. apply env_order_le_lt_trans with (rm (◊ δ1 → δ2) (rm (◊ δ3) Δ) • ◊ δ3 • (◊ δ1 → δ2)).
...
- eapply env_order_equiv_left_compat.
  Unshelve. 3: exact (rm (◊ δ1 → δ2) (rm (◊ δ3) Δ) • (◊ δ1 → δ2) • ◊ δ3).
  + apply Permutation_swap.
  + apply env_order_0'. apply env_order_refl_add. apply remove_In_env_order_refl.
...
Equations a_rule_env_form16  {Δ : list form} (ϕ : form)
  {E : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form}
  {A : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form}
  (θ: form) {Hin : θ ∈ Δ} : form :=
(* A16 *)
...
Equations a_rule_form  {Δ : list form} (ϕ : form)
  {E : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form}
  {A : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form} : form :=
| Var q :=
    if decide (p = q)
...
= (@e_rule_12 Δ ⊥
    (λ pe0 (_ : pe0 ≺· (Δ, ⊥)), EA p true pe0)
    (λ pe0 (_ : pe0 ≺· (Δ, ⊥)), EA p false pe0)) θ₁ θ₂ Hin1 Hin2)).
Proof.
pose (Δ, ϕ) as pe.
remember pe as pe'.
...
- intros θ Hin. cbn in Hin.
  ...
  + simp e_rule ; cbn. f_equal.
    * simp EA. assert (((rm (δ₁ ∨ δ₂) Δ0 • δ₁)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
      f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H ; auto.  
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H1 ; auto.
    * simp EA. assert (((rm (δ₁ ∨ δ₂) Δ0 • δ₂)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
      f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ...
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H1 ; auto.
  + simp e_rule ; cbn. assert (((rm (# q → δ) Δ0 • δ)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    case decide ; intro Hq.
    * simp EA. f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H ; auto.  
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H1 ; auto.
    * case decide ; auto ; intro Hp. simp EA. f_equal. f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H ; auto.  
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H1 ; auto.
  + simp e_rule ; cbn. simp EA. 
    assert (((rm (δ₁ ∧ δ₂ → δ₃) Δ0 • (δ₁ → δ₂ → δ₃))%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
         ...
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H1 ; auto.
  + simp e_rule ; cbn. simp EA.  
    assert (((rm (δ₁ ∨ δ₂ → δ₃) Δ0 • (δ₁ → δ₃) • (δ₂ → δ₃))%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
  ...
  + simp e_rule ; cbn.
    assert (((rm ((δ₁ → δ₂) → δ₃) Δ0 • (δ₂ → δ₃) • δ₁)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    f_equal.
      -- f_equal. simp EA. apply Hind in H as ((H & H0) & H1) ; cbn in H0.
         do 3 (f_equal ; auto).
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ ; auto.
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2 ; auto.
      -- simp EA. assert (((rm ((δ₁ → δ₂) → δ₃) Δ0 • δ₃)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
         apply Hind in H0 as ((H0 & H1) & H2) ; cbn in H0.
         do 3 (f_equal ; auto).
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ ; auto.
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2 ; auto.
  + simp e_rule ; cbn.
    assert ((l_open_boxes (rm (□ δ1 → δ2) Δ0)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    f_equal.
      -- f_equal ; f_equal. simp EA. apply Hind in H as ((H & H0) & H1) ; cbn in H0.
         do 3 (f_equal ; auto).
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ ; auto.
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2 ; auto.
      -- simp EA. assert (((rm (□ δ1 → δ2) Δ0 • δ2)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
         apply Hind in H0 as ((H0 & H1) & H2) ; cbn in H0.
         do 3 (f_equal ; auto).
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ ; auto.
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2 ; auto.
  + simp e_rule ; cbn.
    ...
    f_equal.
      ...
      -- simp EA. assert (((rm (◊ δ1 → δ2) Δ0 • δ2)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
         ...
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2 ; auto.
  + simp e_rule ; cbn.
    assert (((l_open_boxes (rm (□ φ) Δ0) • φ)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    f_equal. simp EA. apply Hind in H as ((H & H0) & H1) ; cbn in H0.
    do 3 (f_equal ; auto).
      -- eapply in_map_ext; intros θ Hθ ; cbn in Hθ ; auto.
      -- eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2 ; auto.
...
- cbn. funelim (e_rule_9 Δ) ; cbn ; cbn in Heqcall ; auto.
  simp e_rule_9 ; cbn. f_equal. destruct (is_box ψ) eqn:E.
  + destruct ψ ; cbn in E ; try discriminate.
    simp EA. assert (((l_open_boxes Δ • ψ)%list, ϕ) ≺· ((Δ • □ ψ)%list, ϕ)) by order_tac.
      f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
         ...
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0. erewrite H1 ; auto.
  + simp EA. assert (((l_open_boxes Δ)%list, ϕ) ≺· ((Δ • ψ)%list, ϕ)) by order_tac.
      f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0.
         destruct ψ ; cbn ; cbn in E ; try discriminate ; erewrite H0 ; auto. 
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0.
            destruct ψ ; cbn ; cbn in E ; try discriminate ; erewrite H ; auto. 
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0.
            destruct ψ ; cbn ; cbn in E ; try discriminate ; erewrite H1 ; auto.
- intros θ1 θ2 Hin1 Hin2. funelim (e_rule_12 θ1 θ2) ; cbn ; cbn in Hin1,Hin2,Heqcall ; auto.
  simp e_rule_12 ; cbn. f_equal.
  + do 2 f_equal. simp EA.
    assert (((l_open_boxes (rm (◊ δ3) (rm (◊ δ1 → δ2) Δ0)) • δ3)%list, ϕ0) ≺· (Δ0, ϕ0)).
    { unfold "≺·" ; cbn. do 2 apply env_order_add_compat.
      apply env_order_le_lt_trans with ((rm (◊ δ3) (rm (◊ δ1 → δ2) Δ0)) • δ3)%list ; order_tac.
      apply env_order_compat ; [ cbn ; lia | ]. rewrite rm_comm. order_tac. }
    f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0 ; auto.
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0 ; auto.
         ++ eapply in_map2_ext; intros θ1 θ2 Hθ1 Hθ2 ; cbn in Hθ1,Hθ2.
            apply Hind in H as ((H & H0) & H1) ; cbn in H0 ; auto.
  + simp EA. 
    assert (((rm (◊ δ1 → δ2) Δ0 • δ2)%list, ϕ0) ≺· (Δ0, ϕ0)) by order_tac.
    f_equal. f_equal.
      -- cbn. apply Hind in H as ((H & H0) & H1) ; cbn in H0 ; auto.
      -- f_equal.
         ++ eapply in_map_ext; intros θ Hθ ; cbn in Hθ.
...
Lemma a_rule_env_form16_cong_strong Δ ϕ θ Hin1 Hin2  E1 A1 E2 A2:
  (forall pe Hpe1 Hpe2, E1 pe Hpe1 = E2 pe Hpe2) ->
  (forall pe Hpe1 Hpe2, A1 pe Hpe1 = A2 pe Hpe2) ->
  @a_rule_env_form16 Δ ϕ E1 A1 θ Hin1 = @a_rule_env_form16 Δ ϕ E2 A2 θ Hin2.
Proof.
intros HeqE HeqA.
destruct ϕ; simp a_rule_env_form16; simpl; trivial; repeat (destruct decide).
destruct θ; simp a_rule_env_form16; simpl; trivial; repeat (destruct decide).
f_equal; repeat erewrite ?HeqE, ?HeqA; trivial.
Qed.

Lemma EA_cong  p Δ ϕ: EA p true (Δ, ϕ) = EA p true (Δ, ⊥).
Proof.
simp EA; simpl. f_equal.
- apply e_rules_cong.
- f_equal. f_equal.
  + apply in_map_ext, e_rules_cong.
  + apply in_map2_ext, e_rules_cong.
Qed.
...
Lemma e_rule_12_vars  Δ (θ1 θ2 : form) (Hin1 : θ1 ∈ Δ) (Hin2 : θ2 ∈ Δ) (ϕ : form)
  (E0 : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form)
  (A0 : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form)
   x
  (HE0 : ∀ pe Hpe,
...
Lemma a_rule_env_vars  Δ θ Hin ϕ
  ...
  (HEA0 : ∀ pe Hpe,
      (occurs_in x (E0 pe Hpe) -> x ≠ p ∧ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ) /\
      (occurs_in x (A0 pe Hpe) -> x ≠ p ∧ (occurs_in x pe.2 \/ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ))):
occurs_in x (@a_rule_env p _ _ E0 A0 θ Hin) -> x ≠ p ∧ (occurs_in x ϕ \/ ∃ θ, θ ∈ Δ /\ occurs_in x θ).
Proof.
...
Lemma a_rule_form_vars  Δ ϕ
  ...
  (HEA0 : ∀ pe Hpe,
      (occurs_in x (E0 pe Hpe) -> x ≠ p ∧ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ) /\
      (occurs_in x (A0 pe Hpe) -> x ≠ p ∧ (occurs_in x pe.2 \/ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ))):
  occurs_in x (@a_rule_form p _ _ E0 A0) -> x ≠ p ∧ (occurs_in x ϕ \/ ∃ θ, θ ∈ Δ /\ occurs_in x θ).
Proof.
destruct ϕ; simp a_rule_form; simpl; try tauto; try solve [repeat case decide; repeat vars_tac].
Qed.

Lemma a_rule_env_form16_vars  Δ θ Hin ϕ
  (E0 : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form)
  (A0 : ∀ pe (Hpe : pe ≺· (Δ, ϕ)), form)
   x
  (HEA0 : ∀ pe Hpe,
      (occurs_in x (E0 pe Hpe) -> x ≠ p ∧ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ) /\
      (occurs_in x (A0 pe Hpe) -> x ≠ p ∧ (occurs_in x pe.2 \/ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ))):
occurs_in x (@a_rule_env_form16 _ _ E0 A0 θ Hin) -> x ≠ p ∧ (occurs_in x ϕ \/ ∃ θ, θ ∈ Δ /\ occurs_in x θ).
Proof.
destruct ϕ; simp a_rule_env_form16; simpl; try tauto; try solve [repeat case decide; repeat vars_tac].
destruct θ; simp a_rule_env_form16; simpl; try tauto; try solve [repeat case decide; repeat vars_tac].
Qed.
...
Lemma a_rule_env17_vars  Δ θ1 θ2 Hin1 Hin2 ϕ
  ...
  (HEA0 : ∀ pe Hpe,
      (occurs_in x (E0 pe Hpe) -> x ≠ p ∧ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ) /\
      (occurs_in x (A0 pe Hpe) -> x ≠ p ∧ (occurs_in x pe.2 \/ ∃ θ, θ ∈ pe.1 /\ occurs_in x θ))):
  occurs_in x (@a_rule_env17 _ _ E0 A0 θ1 θ2 Hin1 Hin2) -> x ≠ p ∧ (occurs_in x ϕ \/ ∃ θ, θ ∈ Δ /\ occurs_in x θ).
Proof.
...
- intros Hocc. destruct Hocc.
  + apply e_rule_9_vars in H ; auto.
    intros pe Hpe. eapply pointed_env_order_bot_R in Hpe.
...
- intro Hocc. destruct Hocc.
  ...
  + apply occurs_in_list_disj in H as (θ & H & H0).
    ...
    * apply elem_of_app in H. destruct H.
      -- apply in_in_map in H as (ψ & Hin & Heq). subst.
         apply a_rule_env_form16_vars in H0 ; auto.
         intros pe Hpe. apply Hind in Hpe.
         destruct (e_rules_cong p pe.1 pe.2) as ((He & He9) & He12).
         rewrite <- He9 in Hpe.
         destruct Hpe. split ; auto.
...
- exch 0. apply OrL; exch 0.
  + apply AndL. exch 0. l_tac. apply ImpL...
...
- destruct Δ ; simp e_rule_9 ; [ cbn ; apply ImpR ; apply ExFalso | ].
  ...
  + order_tac. destruct (is_box f) eqn:E.
    * destruct f ; cbn in E ; try discriminate. order_tac.
    * destruct f ; cbn in E ; try discriminate ; order_tac.
      apply env_order_compat' ; order_tac ; destruct ϕ ; cbn ; lia.
      apply env_order_compat' ; order_tac ; destruct ϕ ; cbn ; lia.
...
- apply list_conjR. intros φ Hin. apply in_app_orT in Hin. destruct Hin as [Hin | Hin].
  + apply elem_of_list_In in Hin. apply in_in_map in Hin.
    ...
    * dependent destruction ψ1; simp e_rule; simpl; auto 3 using HE with proof.
      ...
      -- apply ImpR. exch 0. apply ImpBox.
        ++ rewrite open_boxes_add_t ; cbn ; auto. apply weak_ImpL.
          ** rewrite open_boxes_open_boxes'. unfold open_boxes'. peapply HE ; [order_tac | equivbox ].
    ...
    * apply BoxR. rewrite open_boxes_add_t ; cbn ; auto.
      rewrite open_boxes_open_boxes'. unfold open_boxes'. peapply HE ; [order_tac | ].
    ...
    * apply DiamL. rewrite open_boxes_open_boxes'. unfold open_boxes'. peapply HE ; [order_tac | ].
      repeat rewrite <- list_to_set_disj_env_add. apply env_equiv_eq. f_equal. 
      apply list_to_set_disj_perm ; apply l_open_boxes_perm ; apply gmultiset_elements_list_to_set_disj.
  + apply elem_of_list_In in Hin. apply in_in_map2 in Hin.
    ...
    * rewrite open_boxes_add_t ; cbn ; auto. exch 0. apply weak_ImpL.
      -- repeat rewrite l_open_boxes_rm ; cbn ; auto. peapply HE.
        ++ order_tac. do 2 (apply env_order_compat' ; [ destruct ϕ ; cbn ; lia | ]).
           apply env_order_lt_le_trans with (rm (◊ ψ2) Δ • (◊ ψ2))%list ; [ | apply remove_In_env_order_refl ; apply elem_of_list_In ; auto]. 
      ...
      -- rewrite open_boxes_open_boxes'. unfold open_boxes'. peapply HA.
        ...
        ++ repeat rewrite l_open_boxes_rm ; cbn ; auto. order_tac.
        ...
        ++ apply equiv_disj_union_compat_r.
           ...
           apply l_open_boxes_perm. apply gmultiset_elements_list_to_set_disj.
    * exch 1 ; exch 0. apply weakening. peapply HE ; [ order_tac | ].
...
- apply list_disjL. intros φ Hin. apply in_app_orT in Hin ; destruct Hin as [Hin | Hin].
  ...
  + apply in_app_orT in Hin ; destruct Hin as [Hin | Hin].
    * apply elem_of_list_In in Hin. apply in_in_map in Hin as (φ' & Heq & Hφ'). subst φ.
      destruct ϕ ; simp a_rule_env_form16 ; try now apply ExFalso.
      destruct φ' ; simp a_rule_env_form16 ; try now apply ExFalso.
      cbn. apply elem_of_list_to_set_disj in Heq. exhibit Heq 1. exch 0.
      apply DiamL. rewrite open_boxes_add_t ; cbn ; auto. exch 0 ; apply weak_ImpL.
      -- rewrite EA_cong. peapply HE ; [order_tac | ].
         rewrite open_boxes_open_boxes'. unfold open_boxes'. rewrite <- list_to_set_disj_env_add.
         apply equiv_disj_union_compat_r.
         ...
         apply gmultiset_elements_list_to_set_disj.
      -- rewrite open_boxes_open_boxes'. unfold open_boxes'. peapply HA.
         Unshelve. 3: exact ((l_open_boxes (rm (◊ φ') Δ) • φ')%list). order_tac.
    ...
    * apply elem_of_list_In in Hin. apply in_in_map2 in Hin as (ψ1 & ψ2 & Hin1 & Hin2 & Heq). subst φ.
      ...
      -- destruct (is_box (EA p false ((rm (◊ ψ1_1 → ψ1_2) Δ • ψ1_2)%list, ϕ))) eqn:E.
         ++ rewrite open_boxes_add_t ; cbn ; auto. exch 0. apply weakening.
            rewrite open_boxes_add_t ; cbn ; auto. exch 0. apply weak_ImpL.
            ** rewrite EA_cong. peapply HE.
               --- do 2 (apply env_order_compat' ; [ destruct ϕ ; cbn ; lia | ]). rewrite rm_comm.
                   apply env_order_lt_le_trans with ((rm (◊ ψ2) Δ) • (◊ ψ2))%list ; [ | apply remove_In_env_order_refl ; apply elem_of_list_In ; auto ; order_tac].
                   apply env_order_compat ; cbn ; try lia. rewrite l_open_boxes_rm ; [ apply l_open_boxes_env_order | cbn ; auto].
               --- rewrite open_boxes_open_boxes'. unfold open_boxes'.
            ...
            ** peapply HA. Unshelve. 3: exact ((l_open_boxes (rm (◊ ψ2) (rm (◊ ψ1_1 → ψ1_2) Δ)) • ψ2)%list).
               --- apply env_order_0'. apply env_order_env_order_refl. apply env_order_0'. rewrite rm_comm. apply env_order_env_order_refl.
                    apply env_order_lt_le_trans with (rm (◊ ψ1_1 → ψ1_2) (rm (◊ ψ2) Δ) • (◊ ψ2) • (◊ ψ1_1 → ψ1_2))%list.
                    *** apply env_order_2 ; cbn ; try lia. apply env_order_env_order_refl ; apply env_order_compat ; cbn ; try lia.
                        apply l_open_boxes_env_order.
                    *** apply env_order_refl_trans with ((rm (◊ ψ2) Δ) • (◊ ψ2))%list ;
                        [ | apply remove_In_env_order_refl ; apply elem_of_list_In ; apply elem_of_list_to_set_disj ; auto].
                        apply env_order_refl_trans with (rm (◊ ψ1_1 → ψ1_2) ((rm (◊ ψ2) Δ) • ◊ ψ2) • (◊ ψ1_1 → ψ1_2))%list ;
                        [ cbn ; left | apply remove_In_env_order_refl ; apply elem_of_list_In ; auto].
                        apply elem_of_list_In. right. apply in_in_rm ; auto. apply elem_of_list_In ; apply elem_of_list_to_set_disj ; auto.
      ...
      -- exch 2; exch 1 ; exch 0. apply weakening. exch 1 ; exch 0. peapply HA.
         Unshelve. 3: exact ((rm (◊ ψ1_1 → ψ1_2) Δ • ψ1_2)%list). order_tac.
        apply equiv_disj_union_compat_r. rewrite <- list_to_set_disj_env_add.
...
(Γ • @e_rule p _ _ (λ pe (_ : pe ≺· (Δ, ⊥)),  E p pe.1) (λ pe (_ : pe ≺· (Δ, ⊥)),  A p pe)  φ Hin) ⊢ θ ->
Γ • E p Δ ⊢ θ.
Proof.
intros Hin Hp.
...
(Γ • @e_rule_9 _ _ (λ pe (_ : pe ≺· (Δ, ⊥)),  E p pe.1) (λ pe (_ : pe ≺· (Δ, ⊥)),  A p pe)) ⊢ θ ->
Γ • E p Δ ⊢ θ.
Proof.
unfold E. erewrite <- EA_cong . simp EA. simpl.
intro Hrule.
...
(Γ • @e_rule_12 _ _ (λ pe (_ : pe ≺· (Δ, ⊥)),  E p pe.1) (λ pe (_ : pe ≺· (Δ, ⊥)),  A p pe)  φ1 φ2 Hin1 Hin2) ⊢ θ ->
Γ • E p Δ ⊢ θ.
Proof.
intros Hin1 Hin2 Hp.
unfold E. erewrite <- EA_cong . simp EA. simpl.
...
Γ ⊢ @a_rule_form p _ _ (λ pe (_ : pe ≺· (Δ, φ')), E p pe.1) (λ pe (_ : pe ≺· (Δ, φ')), A p pe) ->
Γ ⊢ A p (Δ, φ').
Proof. 
...
Local Lemma A16_right  {Γ Δ ψ φ'} : ∀ (Hin : ψ ∈ Δ),
Γ ⊢ @a_rule_env_form16 _ _ (λ pe (_ : pe ≺· (Δ, φ')), E p pe.1) (λ pe (_ : pe ≺· (Δ, φ')), A p pe) ψ Hin ->
Γ ⊢ A p (Δ, φ').
Proof. 
intros Hin Hp. unfold A; simp EA; simpl.
assert(Hin' := Hin).
eapply in_map_in in Hin'. destruct Hin' as [Hin' Hrule].
eapply OrR2, list_disjR.
- apply in_or_app ; right ; apply in_or_app ; left ; apply elem_of_list_In ; exact Hrule.
- erewrite a_rule_env_form16_cong_strong.
  + exact Hp.
  + simpl. intros. unfold E. destruct pe. simpl. apply EA_cong.
  + trivial.
Unshelve. exact form_eq_dec.
...
Γ ⊢ @a_rule_env17 _ _ (λ pe (_ : pe ≺· (Δ, φ')), E p pe.1) (λ pe (_ : pe ≺· (Δ, φ')), A p pe) ψ1 ψ2 Hin1 Hin2 ->
Γ ⊢ A p (Δ, φ').
Proof. 
...
eapply OrR2, list_disjR.
- apply in_or_app ; right ; apply in_or_app ; right ; apply elem_of_list_In ; exact Hrule.
...
| Hin : ?a ∈ list_to_set_disj ?Δ  |- _  ⊢ A _ (?Δ, _) =>
...
Local Ltac Atac' := unfold A; simp EA; simpl; simp a_rule_form;
  ...
  (* we want to use the A16 rule *)
Local Ltac A16tac := match goal with
| Hin : ?a ∈ list_to_set_disj ?Δ  |- _  ⊢ A _ (?Δ, _) =>
  apply (A16_right (proj1 (elem_of_list_to_set_disj _ _) Hin)); simp a_rule_env_form16 end.

  (* we want to use the A17 rule *)
Local Ltac A17tac := match goal with
| Hin1 : ?a ∈ list_to_set_disj ?Δ , Hin2 : ?b ∈ list_to_set_disj ?Δ  |- _  ⊢ A _ (?Δ, _) =>
...
- split.
  ...
  + case (decide (p = p0)).
    * intro Heqp0. Atac. do 2 rewrite decide_True by (f_equal; trivial). apply ImpR ; apply ExFalso.
...
- exch 0. apply AndL. exch 1; exch 0. peapply Hind; auto with proof.
  order_tac. occ. peapply' Hp.
- exch 0. apply AndL. exch 1; exch 0. peapply Hind; auto with proof. order_tac. occ. peapply' Hp.
-  split.
   + Etac. foldEA. eapply Hind; auto with proof. order_tac. simpl. peapply' Hp.
   + Atac. Etac. eapply Hind; auto; auto with proof. peapply' Hp.
(* OrR1 & OrR2 *)
- split.
...
(* OrL *)
- exch 0. apply OrL; exch 0.
 + eapply Hind; auto with proof. order_tac. occ. peapply' Hp1.
 + eapply Hind; auto with proof. order_tac. occ. peapply' Hp2.
- exch 0. apply OrL; exch 0.
  + eapply Hind; auto with proof. order_tac. occ. peapply' Hp1.
  + eapply Hind; auto with proof. order_tac. occ. peapply' Hp2.
- split.
  + Etac. apply OrL; eapply Hind; auto with proof; simpl. 1,3: order_tac.
      * peapply' Hp1.
      * peapply' Hp2.
  + Atac. apply weakening. apply AndR.
    * apply ImpR. eapply Hind; auto with proof. order_tac. peapply' Hp1.
    * apply ImpR. eapply Hind; auto with proof. order_tac. peapply' Hp2.
(* ImpR *)
- split.
  ...
  + intro Hin0.
    ...
    * assert(Hin0' : Var p0 ∈ (Γ0•Var p0•(Var p0 → φ))) by ms. rewrite Heq in Hin0'.
      ...
      -- apply contraction. Etac. rewrite decide_False by tauto.
          ...
          exch 0. foldEA. eapply Hind; auto with proof. order_tac. occ. peapply' Hp.
  + intro.
    ...
    * (* subcase 2: p0 ∈ Γ ; (p0 → φ) ∈ Δ *)
      ...
      -- Etac. case decide; intro Hp0;[|case decide; intro; subst; [auto with *|]].
         ++ foldEA. simpl. eapply Hind; auto with proof. order_tac. apply env_order_add_compat.
            apply env_order_disj_union_compat_strong_left ; auto. left ; auto. order_tac.
            occ. peapply' Hp.
         ++ apply ImpLVar. exch 0. backward. rewrite env_add_remove.
            eapply Hind; auto with proof. order_tac. simpl. peapply' Hp.
      -- Etac. Atac. case decide; intro Hp0;[|case decide; intro; subst; [auto with *|]].
        ++ foldEA. eapply Hind; auto with proof; [order_tac | occ | peapply' Hp].
           apply env_order_add_compat. apply env_order_disj_union_compat_strong_left ; auto. left ; auto. order_tac.
        ++ apply ImpLVar. apply AndR; auto 2 with proof. exch 0. backward. rewrite env_add_remove.
           eapply Hind; auto with proof. order_tac. peapply' Hp.
    * assert(Hin': Var p0 ∈ Γ ⊎ list_to_set_disj Δ) by (rewrite <- Heq; ms).
      ...
      -- (* subsubcase p = p0 *)
        ...
        ++ clear Heq. eapply Hind; auto with proof. order_tac. simpl.  peapply' Hp.
      ...
      -- (* subsubcase p ≠ p0 *)
         ...
         ++ apply contraction. Etac. rewrite decide_False by trivial. exch 0.
            ...
            exch 0. apply weakening. eapply Hind; auto with proof. order_tac. simpl. peapply' Hp.
         ++ apply contraction. Etac. exch 0. assert((#p0 → φ) ∈ list_to_set_disj Δ) by ms.
            ...
            eapply Hind; auto with proof. order_tac. peapply' Hp.
(* ImpLAnd *)
- exch 0. apply ImpLAnd. exch 0. eapply Hind; auto with proof; [order_tac | occ |peapply' Hp].
- exch 0. apply ImpLAnd. exch 0. eapply Hind; auto with proof; [order_tac | occ |peapply' Hp].
- split; Etac.
  + eapply Hind; auto with proof. simpl. order_tac. peapply' Hp.
    rewrite list_to_set_disj_rm. ms.
  + Atac. simpl. eapply Hind; auto with proof. order_tac. peapply' Hp.
(* ImpLOr *)
- exch 0; apply ImpLOr. exch 1; exch 0. eapply Hind; auto with proof. order_tac. occ. peapply' Hp.
- exch 0; apply ImpLOr. exch 1; exch 0. eapply Hind; auto with proof ; [order_tac | occ|peapply' Hp].
- split; Etac.
  + eapply Hind; auto with proof. order_tac. simpl. peapply' Hp.
  + Atac. eapply Hind; auto with proof. order_tac. peapply' Hp.
(* ImpLImp *)
...
- (* subcase 2: ((φ1 → φ2) → φ3) ∈ Δ *)
  ...
  + Etac. simpl. apply ImpLImp.
    ...
    * eapply Hind; auto with proof. order_tac. simpl. peapply' Hp2.
  + Atac. apply AndR.
    ...
    * Etac. simpl. apply ImpLImp.
      -- apply weakening. apply ImpR. foldEA.
       ...
       ++ repeat setoid_rewrite gmultiset_disj_union_assoc.
          ...
          peapply' Hp1.
      -- eapply Hind; auto with proof. order_tac. peapply' Hp2.   
(* ImpBox *)
- exch 0. apply weak_ImpL.
  + apply E9_left. apply BoxR. destruct Δ ; simp e_rule_9.
    ...
    * rewrite open_boxes_add_t ; auto. cbn. eapply Hind; auto with proof.
      -- order_tac. apply env_order_0'. apply env_order_env_order_refl.
         apply env_order_0'. apply env_order_env_order_refl.
         apply env_order_2 ; try (cbn ; lia). apply env_order_refl_disj_union_compat.
  ...
  + exch 0. peapply Hind ; auto with proof ; [ order_tac | occ | ].
...
- split ; [intro Hocc | ].
  + Etac. cbn. apply weak_ImpL.
    * apply BoxR,ImpR. eapply Hind ; auto with proof.
      -- order_tac. do 2 (apply env_order_cancel_right). do 2 (rewrite Permutation_middle).
      ...
      -- peapply Hp1. rewrite l_open_boxes_rm ; auto.
        ...
        ++ rewrite <- H. rewrite Heq. rewrite open_boxes_disj_union. apply proper_disj_union ; auto.
            ...
            apply list_to_set_disj_perm. rewrite <- l_open_boxes_open_boxes_perm. auto.
        ++ rewrite open_boxes_add_f ; auto.
    * peapply Hind ; auto with proof ; [ order_tac | ].
    ...
    * case (decide ((◊ χ) ∈ Γ)); intro Hin2.
      ...
      -- apply contraction. exhibit Hin1 2.
         ...
         ++ destruct Δ ; simp e_rule_9.
            ...
            ** remember (l_open_boxes (Δ • f)) as Δ' ; cbn.
               ...
               --- rewrite open_boxes_add_t ; cbn ; auto ; destruct (CKPropQuantifiers.E p (Δ • f)) ; cbn in E ; try discriminate ; cbn.
                   exch 1 ; exch 0. apply weakening ; exch 0. eapply Hind; auto with proof.
                   +++ order_tac. do 2 (apply env_order_cancel_right). do 4 (rewrite Permutation_middle).
                       ...
                       *** apply env_order_disj_union_compat_strong_right.
                           ---- eapply env_order_lt_le_trans ; [ | apply remove_In_env_order_refl with (φ:=(◊ φ1 → φ2)) ].
                                { apply env_order_2 ; cbn ; try lia. rewrite <- elements_setminus.
                                  assert ((⊗ Γ ∖ {[◊ φ1 → φ2]} ∖ {[◊ χ]}) ≡ (⊗ Γ ∖ {[◊ χ]} ∖ {[◊ φ1 → φ2]})).
                                  { repeat rewrite open_boxes_remove_f ; auto. apply in_difference ; auto. }
                                  rewrite H. apply elements_open_boxes_order. }
                                { apply elem_of_list_In. apply gmultiset_elem_of_elements. apply in_difference ; auto. }
                           ---- apply env_order_env_order_refl. apply env_order_1' ; [cbn ; try lia | ]. apply l_open_boxes_env_order.
               ...
               --- rewrite open_boxes_add_f ; cbn ; auto.
                   ...
                   +++ order_tac. do 2 (apply env_order_0' ; apply env_order_env_order_refl).  do 4 (rewrite Permutation_middle).
                       apply (@env_order_equiv_left_compat _ _ ((elements (⊗ Γ ∖ {[◊ φ1 → φ2]} ∖ {[◊ χ]}) • φ1 • φ1) ++ (l_open_boxes (Δ • f) • χ))).
                       *** do 2 rewrite (perm_swap χ). do 3 rewrite -> Permutation_cons_append ; cbn. do 2 rewrite Permutation_middle.
                           apply Permutation_app_head. do 3 rewrite <- Permutation_middle ; cbn.
                          repeat rewrite app_nil_r.  do 2 rewrite (perm_swap χ). auto.
         ...
         ++ exch 1 ; exch 0. apply weakening. exch 1 ; exch 0. eapply Hind; auto with proof.
            ** order_tac. apply (@env_order_equiv_left_compat _ _ ((elements (Γ ∖ {[◊ φ1 → φ2]} ∖ {[◊ χ]}) • φ2) ++ (Δ • ◊ χ))).
               ...
               --- rewrite Permutation_middle. apply env_order_disj_union_compat_strong_right ; [ | left ; auto].
                   eapply env_order_lt_le_trans ; [ | apply remove_In_env_order_refl with (φ:=(◊ φ1 → φ2)) ].
                   { apply env_order_1' ; cbn ; try lia. rewrite <- elements_setminus.
                     assert ((Γ ∖ {[◊ φ1 → φ2]} ∖ {[◊ χ]}) ≡ (Γ ∖ {[◊ χ]} ∖ {[◊ φ1 → φ2]})).
      ...
      -- apply contraction.
         ...
         ++ destruct (is_box (E p Δ)) eqn:E.
            ** rewrite open_boxes_add_t ; cbn ; auto ; destruct (CKPropQuantifiers.E p Δ) ; cbn in E ; try discriminate ; cbn.
               exch 0 ; apply weakening. eapply Hind; auto with proof.
               --- order_tac. do 2 (apply env_order_0' ; apply env_order_env_order_refl). 
                   apply env_order_2 ; try (cbn ; lia). apply env_order_refl_disj_union_compat. 
                   +++ apply elements_open_boxes_order.
    ...
    * case (decide ((◊ χ) ∈ Γ)); intro Hin2.
      ...
      -- apply contraction.
         ...
         ++ apply ImpR. destruct (is_box (E p Δ)) eqn:E.
            ** rewrite open_boxes_add_t ; cbn ; auto ; destruct (CKPropQuantifiers.E p Δ) ; cbn in E ; try discriminate ; cbn.
               ...
               --- order_tac. do 2 (apply env_order_0' ; apply env_order_env_order_refl). 
                   do 4 (rewrite Permutation_middle). apply env_order_disj_union_compat_strong_left.
                   +++ apply elements_open_boxes_order.
            ...
            ** rewrite open_boxes_add_f ; cbn ; auto.
               ...
               --- erewrite proper_Provable ; [ exact Hp1 | | reflexivity].
                   ...
                   { apply gmultiset_elem_of_disj_union ; right. apply elem_of_list_to_set_disj ; auto. }
         ++ exch 1 ; exch 0 ; apply weakening. eapply Hind; auto with proof.
            ** order_tac. apply env_order_add_compat. apply env_order_disj_union_compat_strong_left.
               --- left ; auto.
      ...
      -- assert (Hin3: (◊ φ1 → φ2) ∈ Δ).
         ...
         ++ apply ImpR. eapply Hind; auto with proof.
            ** order_tac. do 2 (apply env_order_0' ; apply env_order_env_order_refl).
               do 2 (rewrite Permutation_middle). apply env_order_disj_union_compat_strong_left. 
               --- apply elements_open_boxes_order.
               --- eapply env_order_lt_le_trans ; [ | apply remove_In_env_order_refl with (φ:=(◊ φ1 → φ2)) ; apply elem_of_list_In in Hin3 ; auto].
  ...
  + case (decide ((◊ φ1 → φ2) ∈ Γ)); intro Hin1.
      ...
      -- exhibit Hin1 1.
               ...
               --- rewrite open_boxes_add_t ; cbn ; auto ; destruct (CKPropQuantifiers.E p (Δ • f)) ; cbn in E ; try discriminate ; cbn.
                   ...
                   +++ order_tac. do 2 (apply env_order_0' ; apply env_order_env_order_refl).
                       ...
                       *** apply env_order_1' ; [ cbn ; try lia | ]. apply env_order_env_order_refl.
                           ...
                           eapply env_order_lt_le_trans ; [ | apply remove_In_env_order_refl with (φ:=(◊ φ1 → φ2)) ].
                           { do 2 rewrite elements_env_add. apply env_order_2 ; cbn ; try lia. rewrite <- elements_setminus.
                             assert ((⊗ Γ ∖ {[◊ φ1 → φ2]} ∖ {[◊ χ]}) ≡ (⊗ Γ ∖ {[◊ χ]} ∖ {[◊ φ1 → φ2]})).
                             { repeat rewrite open_boxes_remove_f ; auto. apply in_difference ; auto. }
                             rewrite H. apply elements_open_boxes_order. }
                           { apply elem_of_list_In. apply gmultiset_elem_of_elements. apply in_difference ; auto. }  
         ...
         ++ exch 1 ; exch 0. apply weakening. exch 1 ; exch 0. eapply Hind; auto with proof.
            ** order_tac. apply (@env_order_equiv_left_compat _ _ (elements ((Γ ∖ {[◊ φ1 → φ2]} ∖ {[◊ χ]}) • φ2) ++ Δ • ◊ χ)).
               ...
               --- apply env_order_add_compat.
                   apply env_order_disj_union_compat_strong_right ; [ | left ; auto].
                   rewrite (@elements_elem_of (Γ ∖ {[◊ χ]}) (◊ φ1 → φ2)) ; [ | apply in_difference ; auto].
    ...
    * case (decide ((◊ χ) ∈ Γ)); intro Hin2.
      ...
      -- assert (Hin3: (◊ φ1 → φ2) ∈ Δ).
         ...
         ++ apply ImpR. eapply Hind; auto with proof.
            ** order_tac. do 2 (apply env_order_0' ; apply env_order_env_order_refl). 
               apply (@env_order_equiv_left_compat _ _ (elements ((⊗ Γ ∖ {[◊ χ]})) ++ l_open_boxes (rm (◊ φ1 → φ2) Δ) • φ1 • φ1 • χ)).
                       *** repeat rewrite (perm_swap χ) ; auto.
                       *** apply env_order_1' ; [cbn ; try lia | ].
            ...
            ** apply weakening,DiamL,ImpR. eapply Hind; auto with proof.
              ...
              --- erewrite proper_Provable ; [ exact Hp1 | | reflexivity].
                  assert ((⊗ Γ0) ≡ ⊗ (Γ0 • ◊ χ)).
                   ...
                   { apply gmultiset_elem_of_disj_union ; right. apply elem_of_list_to_set_disj ; auto. }
            ** eapply Hind; auto with proof.
              --- order_tac. apply env_order_add_compat. order_tac.
              --- occ.
              --- erewrite proper_Provable ; [ exact Hp2 | | reflexivity].
                  rewrite Heq'. rewrite union_difference_R ; auto.
                  +++ rewrite <- difference_singleton ; auto. rewrite list_to_set_disj_rm. ms.
...
- split. 
  + intro H. assert (◊ ψ ∈ Δ) by (apply elem_of_list_to_set_disj ; ms).
    ...
    eapply Hind; auto with proof. 
    * order_tac. apply env_order_1' ; try (cbn ; lia) ; apply env_order_env_order_refl.
      apply env_order_1' ; try (cbn ; lia). apply env_order_refl_disj_union_compat.
      -- apply elements_open_boxes_order.
      -- apply env_order_env_order_refl.
    ...
    * (erewrite proper_Provable;  [| |reflexivity]);  [eapply Hp|].
      ...
      rewrite open_boxes_disj_union. rewrite list_to_set_disj_open_boxes.
      rewrite <- l_open_boxes_open_boxes_perm. ms.
  + A16tac ; cbn. apply weakening. apply BoxR,ImpR. eapply Hind; auto with proof.
    * order_tac. apply env_order_1' ; try (cbn ; lia) ; apply env_order_env_order_refl.
      apply env_order_1' ; try (cbn ; lia). apply env_order_refl_disj_union_compat.
      -- apply elements_open_boxes_order.
      -- apply env_order_env_order_refl.
...
Theorem CK_uniform_interpolation  p V: p ∉ V ->
  ∀ φ, vars_incl φ (p :: V) ->
    (vars_incl (Ef p φ) V)
  * ({[φ]} ⊢ (Ef p φ))
