...
fun x => Δ x \/ (Γ x /\ ~ (CKH_prv (Union _ Δ (Singleton _ x)) ψ) /\ φ = x).
...
Lemma der_nLind_theory_mLind_theory_le : forall n m Γ Δ ψ φ,
  (CKH_prv (nLind_theory Γ Δ ψ n) φ) -> (le n m) ->
    (CKH_prv (nLind_theory Γ Δ ψ m) φ).
Proof.
intros. apply (CKH_monot _ _ H) ; auto.
intros C HC ; apply nLind_theory_extens_le with (n:=n) ; auto.
Qed.

Lemma der_Lind_theory_nLind_theory : forall Γ Δ ψ A, CKH_prv (Lind_theory Γ Δ ψ) A ->
                                        exists n, CKH_prv (nLind_theory Γ Δ ψ n) A.
Proof.
...
eapply MP. 2: exact H0. apply CKH_Deduction_Theorem in H2.
apply (CKH_monot _ _ H2). intros C HC. exists (form_index A) ; auto.
Qed.
...
Lemma not_In_Lind_theory_deriv : forall Γ Δ ψ,
  ...
  forall A, Γ A -> ~ (Lind_theory Γ Δ ψ A) ->
    ~~ CKH_prv (Union _ (Lind_theory Γ Δ ψ) (Singleton _ A)) ψ.
Proof.
...
intro. apply H1. apply (CKH_monot _ _ H2). intros C HC. unfold In in *. inversion HC ; subst.
...
apply CKH_monot with (Γ1:=Union form (Lind_theory (Clos Γ) Δ ψ) (Singleton form a)) in H. exact H.
...
apply CKH_Deduction_Theorem.
apply CKH_monot with (Γ1:=Union form (Lind_theory (Clos Γ) Δ ψ) (Singleton form b)) in H2. exact H2.
