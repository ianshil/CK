...
Definition AdAxk4Ndb := fun x => AdAx x \/ (Ndb = x \/ exists A B, (k4 A B) = x).

Lemma negneg_box_prv : forall Γ φ, extCKH_prv AdAxk4Ndb Γ ((¬ ☐ ⊥) --> (¬ ¬ ☐ φ) --> ☐ ¬ ¬ φ).
Proof.
intros.
...
eapply MP. apply And_Imp.
eapply MP. eapply MP. apply Imp_trans.
2: apply Ax ; left ; left ; eapply IA9 ; reflexivity.
...
split.
- eapply MP. eapply MP. apply Imp_trans.
  apply more_AdAx_more_prv with (AdAxk4Ndb (fun _ : form => False)).
